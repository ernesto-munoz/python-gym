...
pad_with_spaces = lambda text, length, align="left": text + " " * (length - len(text)) if align == "left" else " " * (
...
def print_row(descr, pyglm, numpy, ratio, print_header=True, end="\n"):
    ...
    print(
        f"| {pad_with_spaces(descr_last_line, 38)} | {pad_with_spaces(pyglm, 10, align='right')} | {pad_with_spaces(numpy, 10, align='right')} | {pad_with_spaces(ratio, 9, align='right')} |",
        end=end)
...
def run_test(descr, pyglm_setup_code, pyglm_code, numpy_setup_code, numpy_code, number):
    ...
    for i in range(10):
        ...
        if ON_WINDOWS:
            print_row(descr, "{}ms".format(seconds_to_milliseconds(pyglm_result)),
                      "{}ms".format(seconds_to_milliseconds(numpy_result)),
                      "{:.02f}x".format(numpy_result / pyglm_result), end="\r", print_header=False)

    pyglm_total_time += pyglm_result
    numpy_total_time += numpy_result

    print_row(descr, "{}ms".format(seconds_to_milliseconds(pyglm_result)),
              "{}ms".format(seconds_to_milliseconds(numpy_result)), "{:.02f}x".format(numpy_result / pyglm_result),
              print_header=not ON_WINDOWS)
